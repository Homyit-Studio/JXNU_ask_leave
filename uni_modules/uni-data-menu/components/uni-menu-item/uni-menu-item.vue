<template>
	<view class="uni-menu-item" :class="{'is-active':active,'is-disabled':disabled}"
		:style="{'color':disabled?'#999':(active?'000':textColor),'background-color':active? '#fff':''}"
		@click="onClickItem">
		<slot></slot>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			paddingLeft() {
				return 20 * this.rootMenu.SubMenu.length + 'px'
			}
		},
		created() {
			this.init()
