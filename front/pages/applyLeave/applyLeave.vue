<template>
	<view>
		...
		<view class="apply-leave">
			<view>
				<uni-forms ref="form" :modelValue="formData" :rules="dataRules" class="form-style" :border="true"
					...
					<uni-group>
						<uni-forms-item required name="destination" label="目的地">
							<uni-easyinput multiple v-model="formData.destination" placeholder="请输入目的地"
								:inputBorder="false" />
						</uni-forms-item>
					</uni-group>
					<uni-group>
						<uni-forms-item required name="startTime" label="开始时间">
							<uni-datetime-picker type="datetime" v-model="formData.startTime" :border="false"
								:clear-icon="false" placeholder="选择离校日期和时间" />
						</uni-forms-item>
					</uni-group>
					<uni-group>
						<uni-forms-item required name="endTime" label="结束时间">
							<uni-datetime-picker type="datetime" v-model="formData.endTime" :border="false"
								:clear-icon="false" placeholder="选择返校日期和时间" />
						</uni-forms-item>
					</uni-group>
					<uni-group>
					...
					<uni-group>
						<uni-forms-item required name="reason" label="申请事由">
							<view class="switch">
								<uni-easyinput type="textarea" autoHeight v-model="formData.reason" placeholder="请输入内容"
									:inputBorder="false" class="input-textarea"></uni-easyinput>
							</view>
						</uni-forms-item>
					</uni-group>
				</uni-forms>
				<view class="button-group">
					<button type="button" @click="submitForm" size="mini">提交</button>
				</view>
			</view>
				...
				this.$refs.form.validate().then(res => {
					uni.$http.post('/leave/ask', {
						...
						if (res.data.code === 200) {
							...
							//this.uploadImg()
							uni.redirectTo({
								url: '../finishLeave/finishLeave?formData=' + encodeURIComponent(
